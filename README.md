# EnderalVR-VanillaPlus

## Introduction:

This guide describes how to configure Skyrim VR to play Enderal: Forgotten Stories SE v2.0.11 in VR using Wabbajack (v2.0.12 is not VR compatible). Note that the ONLY version of Skyrim you need is Skyrim VR. T

## Requirements

- [The Elder Scrolls V: Skyrim VR](https://store.steampowered.com/app/611670/The_Elder_Scrolls_V_Skyrim_VR/)
- Be registered in https://mod.pub/ (to download the Enderal Mod)
- [Enderal SE Mod](https://mod.pub/enderal-se/38/files)
- [Wabbajack](https://www.wabbajack.org/#/)
- [EnderalVR-VanillaPlus](https://www.nexusmods.com/enderalspecialedition/mods/993)

## Installation

1. Meet the requirements
2. Install Skyrim VR.
3. Run Skyrim VR one time from Steam and then exit the game.
4. Create a new folder where you wish to install Enderal VR i.e. C:\Games\EnderalVR
5. Launch Wabbajack and select "Install from Disk". Next, manually select the .wabbajack file under "Target Modlist".
6. Under "Installation Location", select the folder you created above under Step 4.
7. Click the Install Button.
8. Wabbajack will ask you to download the Enderal Mod File, copy the file linked in the requirements to the download folder. i.e. C:\Games\EnderalVR\downloads
9. Copy the contents of C:\Games\EnderalVR\Game Folder Files\ to C:\Program Files (x86)\Steam\steamapps\common\SkyrimVR
10. Launch Mod Organizer 2.

## Modlist

- [ANIM] CGO Stripped - Jump Attacks Leaning and Camera Noise ONLY
- [ANIM] Ice skating fixed for real - No more attack sliding movement (NEMESIS compatible)
- [ANIM] Neutral VR Animations for VRIK and PCEA2
- [ANIM] Project New Reign - Nemesis PCEA
- [ANIM] Project New Reign - Nemesis Unlimited Behavior Engine
- [ANIM] Super Fast Get Up Animation

- [CLUT] Enhanced Textures Detail (UV-tweaks)

- [CORE] Animated Static Reload Fix - NG
- [CORE] Animation Motion Fix
- [CORE] Animation Queue Fix
- [CORE] Blank Binks - Null Videos
- [CORE] Dialogue Movement Enabler VR
- [CORE] Disk Cache Enabler
- [CORE] Enderal SE (VR compatible)-38-2.0.11-2024.08.02-[mod.pub]
- [CORE] Enderal SE - Bug Fixes
- [CORE] Enderal VR
- [CORE] Engine Fixes VR
- [CORE] Increase Actor Limit for VR
- [CORE] JContainers VR
- [CORE] LeveledList Crash Fix VR
- [CORE] Papyrus Extender VR
- [CORE] Poached Bugs VR
- [CORE] Poached Bugs VR - Simonrim Choice Config 0.5.0
- [CORE] powerofthree's Papyrus Extender
- [CORE] Recursion Monitor
- [CORE] Save Unbaker VR
- [CORE] Skyrim Priority SE AE - CPU Performance FPS Optimizer
- [CORE] Skyrim Script Extender for VR (SKSEVR)
- [CORE] Skyrim VR ESL Support
- [CORE] SkyrimVR.ESM DynDOLOD Patch
- [CORE] SkyUI [1.2.2]
- [CORE] Stagger Effect Fix
- [CORE] VR Address Library for SKSEVR

- [CS] Cloud Shadows - Community Shaders
- [CS] Community Shaders
- [CS] Crash Logger SSE AE VR - PDB support
- [CS] Extended Translucency - Community Shaders
- [CS] Grass Collision - Community Shaders
- [CS] Grass Lighting - Community Shaders
- [CS] Inverse Square Lighting - Community Shaders
- [CS] Sky Sync - Community Shaders
- [CS] Skylighting - Community Shaders
- [CS] Subsurface Scattering - Community Shaders
- [CS] Terrain Blending - Community Shaders
- [CS] Terrain Shadows - Community Shaders [HEAVY PERFORMANCE]
- [CS] Terrain Variation - Community Shaders
- [CS] Upscaling - Community Shaders
- [CS] Water Effects - Community Shaders
- [CS] Wetness Effects - Community Shaders

- [ECO] Grass FPS Booster
- [ECO] Storm Lightning for SSE and VR (Minty Lightning 2019)

- [EXT] Behavior Data Injector
- [EXT] Behavior Data Injector Universal Support
- [EXT] Centered Messages Box VR
- [EXT] Combat Pathing Revolution
- [EXT] Combat Pathing Revolution VR
- [EXT] Conduit - Enderal Adaption Patch
- [EXT] Conduit - Short-term Weapon Infusions
- [EXT] ConsolePlusPlus VR
- [EXT] Dynamically Disable Eye Adaptation and Bloom
- [EXT] Essential Favorites VR
- [EXT] Locational Damage SKSE VR
- [EXT] More Informative Console
- [EXT] moreHUD VR
- [EXT] No More Eagle Eye Crosshair
- [EXT] No Stagger Mod
- [EXT] Parrying RPG
- [EXT] Physical Dodge VR
- [EXT] Quickloot VR
- [EXT] Rally's Candlelight and Magelight Fix
- [EXT] Simple Offence Suppression
- [EXT] Simple Offence Suppression VR
- [EXT] SkyUI Colored Category Icons SE
- [EXT] Splashes of Skyrim
- [EXT] Splashes of Skyrim VR
- [EXT] Splashes Of Storms
- [EXT] Splashes of Storms VR
- [EXT] To Your Face SE - AE - VR
- [EXT] Transparent Dialogue VR

- [FX] 3D Casting FX - Fire and Frost Hand FX Replacer

- [VR] Dual Wield Block VR
- [VR] Haptic Skyrim VR - Spellcasting and Enhanced Bow and Melee Haptics
- [VR] HIGGS - Enhanced VR Interaction
- [VR] Instant Equip VR
- [VR] Magic Improvements for Skyrim VR
- [VR] PLANCK - Physical Animation and Character Kinetics
- [VR] Skyrim VR Tools
- [VR] Spell Wheel VR
- [VR] Sprint Jump VR
- [VR] Subtitle Dialog HUD resize for VR
- [VR] vrik neardistance fix patch for build 28
- [VR] VRIK Player Avatar

- [LOD] DynDOLOD 
 
